...
import { Sparkles, MapPin, Building, Briefcase, Globe, ArrowLeft, CheckCircle2, XCircle, ChevronRight, Zap, LogOut } from 'lucide-react';
...
const JobDetailPage = () => {
  ...
  const [job, setJob] = useState<JobMock | null>(null);
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center font-sans tracking-wide">
        <div className="text-center">
          <h2 className="text-3xl font-heading font-bold text-text-primary mb-4">Job Not Found</h2>
          <button 
  ...
  const matchedSkills = job.skills.filter(s => s.status === 'matched');
  const missingSkills = job.skills.filter(s => s.status === 'missing');

  return (
    <div className="min-h-screen bg-background text-text-primary flex flex-col font-sans selection:bg-primary-500/30">
      {/* Navigation Block (reused from DashboardPage for consistency) */}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                <div>
                  <h1 className="text-3xl md:text-4xl font-heading font-black text-text-primary mb-4 tracking-tight">{job.title}</h1>
                ...
                <div className="flex-shrink-0 flex flex-col items-center justify-center p-5 rounded-3xl bg-primary-500/5 border border-primary-500/20 shadow-glow relative overflow-hidden group">
                  ...
                  <span className="font-heading font-black text-5xl text-primary-400 leading-none relative z-10">{job.matchScore}%</span>
                  <span className="text-[10px] uppercase tracking-widest font-bold mt-2 text-primary-500/80 relative z-10">Match Score</span>
                </div>
              </div>

              <div className="mt-8 pt-8 border-t border-background-elevated">
                <h2 className="text-xl font-heading font-bold text-text-primary mb-4">About the Role</h2>
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-3 flex items-center justify-between">
                    Matched Skills
                    <span className="bg-primary-500/20 text-primary-400 px-2 py-0.5 rounded-md text-[10px]">{matchedSkills.length}</span>
                  </h4>
                  {matchedSkills.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {matchedSkills.map((skill, i) => (
                        <span key={i} className="px-3 py-1.5 bg-primary-500/10 text-primary-400 rounded-xl text-sm border border-primary-500/20 flex items-center shadow-sm">
                    ...
                    <p className="text-sm text-text-muted bg-background/50 p-3 rounded-xl border border-white/5">No key skills matched directly.</p>
                  )}
                </div>
                ...
                <div className="pt-4 border-t border-background-elevated">
                  <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-3 flex items-center justify-between">
                    Missing Skills
                    <span className="bg-red-500/20 text-red-400 px-2 py-0.5 rounded-md text-[10px]">{missingSkills.length}</span>
                  </h4>
                  {missingSkills.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {missingSkills.map((skill, i) => (
                        <span key={i} className="px-3 py-1.5 bg-red-500/10 text-red-400 rounded-xl text-sm border border-red-500/20 flex items-center shadow-sm">
            ...
            {missingSkills.length > 0 && (
              ...
              >
                ...
                <div className="space-y-4 relative z-10">
                  {missingSkills.map((skill, i) => (
                    <div key={i} className="bg-background-elevated/40 border border-white/5 rounded-2xl p-4 hover:bg-background-elevated/60 transition-colors">
                      ...
                      <p className="text-sm text-text-secondary leading-relaxed">
                        {skill.improvementTip || 'Consider adding a project or course demonstrating your knowledge of this skill.'}
                      </p>
                    </div>
                  ))}
                </div>
